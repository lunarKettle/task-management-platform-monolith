...
func (r *UserRepository) Create(user models.User) (uint32, error) {
	...
	var userID uint32
	err := r.db.connection.QueryRow(query, user.Username, user.Email, user.PasswordHash, user.Role).Scan(&userID)
	if err != nil {
		return 0, err
	}
...
func (r *UserRepository) Update(user models.User) error {
	...
	result, err := r.db.connection.Exec(query, user.Username, user.Email, user.PasswordHash, user.Role, user.ID)
	if err != nil {
		return err
	}
